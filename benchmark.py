...
def get_model(name, res, width, modes, in_ch=1, out_ch=1):
    ...
    if name == 'FNO':
        ...
        return FNO(n_modes=(modes, modes), 
    ...
    # --- TFNO (Tensorized FNO) ---
        ...
        return TFNO(n_modes=(modes, modes), 
                    hidden_channels=width, 
                    in_channels=in_ch, 
                    out_channels=out_ch,
                    factorization='tucker', # Standard default
                    rank=0.42) # Common compression ratio
...
#                       out_channels=out_ch)
    ...
    # U-NO (U-Shaped)
        ...
        # UNO doesn't use "modes" in the same way, it relies on scaling
        # We use 'width' for hidden_channels
        return UNO(in_channels=in_ch, 
                   out_channels=out_ch, 
                   hidden_channels=width, 
    ...
    # CODANO (Attention-based)
        ...
        return CODANO(in_channels=in_ch, 
                      out_channels=out_ch,
                      hidden_channels=width,
                      n_modes=(modes, modes)) 
